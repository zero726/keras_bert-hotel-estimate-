...
def get_train_test_data():
    train_df = pd.read_excel('./data/data_train.xlsx').astype(str)
    test_df = pd.read_excel('./data/data_test.xlsx').astype(str)
...
# 预训练好的模型 bert base
config_path = './bert/bert_config.json'  # 加载配置文件
checkpoint_path = './bert/bert_model.ckpt'
# dict_path = r'C:\Users\86188\Desktop\nlp\bert_keras_nlp\bert\vocab.txt'
...
def run_kb():
    ...
    # 当评价指标不在提升时，减少学习率,verbose = 1 为输出进度条记录  verbose =2：为每个epoch输出一行记录
    checkpoint = ModelCheckpoint('bert_dump1.hdf5', monitor='val_acc', verbose=2,
